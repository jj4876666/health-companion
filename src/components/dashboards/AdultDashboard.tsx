...
export function AdultDashboard() {
  ...
  const baseUser = { ...emptyDefaults, ...(currentUser || {}) };
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" asChild>
            <Link to="/emergency"><Phone className="w-4 h-4 mr-2" />Emergency</Link>
          </Button>
          <Button variant="outline" asChild>
            <Link to="/consultation"><Stethoscope className="w-4 h-4 mr-2" />Consult</Link>
          </Button>
          <Button asChild>
            <Link to="/education"><BookOpen className="w-4 h-4 mr-2" />Education</Link>
          </Button>
        </div>
      </div>
      ...
      <div className="p-3 rounded-lg bg-primary/10 border border-primary/20 text-center">
        <p className="text-sm text-primary font-medium">
          🏥 Your medical records are managed by your Health Officer
        </p>
      </div>
      ...
      <Tabs defaultValue="records" className="w-full">
        <TabsList className="grid w-full grid-cols-7">
          <TabsTrigger value="records" className="gap-2"><FileText className="w-4 h-4" />Records</TabsTrigger>
          <TabsTrigger value="ai" className="gap-2"><Bot className="w-4 h-4" />AI Chat</TabsTrigger>
          <TabsTrigger value="targets" className="gap-2"><Target className="w-4 h-4" />Goals</TabsTrigger>
          <TabsTrigger value="profile" className="gap-2"><User className="w-4 h-4" />Profile</TabsTrigger>
          <TabsTrigger value="health" className="gap-2"><Heart className="w-4 h-4" />Health</TabsTrigger>
          <TabsTrigger value="medications" className="gap-2"><Pill className="w-4 h-4" />Meds</TabsTrigger>
          <TabsTrigger value="allergies" className="gap-2"><AlertTriangle className="w-4 h-4" />Allergies</TabsTrigger>
        </TabsList>

        <TabsContent value="records" className="space-y-4 mt-4">
          {profileId && <LiveMedicalUpdates profileId={profileId} />}
        ...
        <TabsContent value="profile" className="space-y-4 mt-4">
          <Card className="border-0 shadow-elegant">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><CreditCard className="w-5 h-5" />Patient Information</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="p-3 rounded-lg bg-muted/50">
                  ...
                  <p className="font-bold text-lg">{adult.age ? `${adult.age} years` : 'Not set'}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/50">
                  <p className="text-sm text-muted-foreground">Blood Group</p>
                  <p className="font-bold text-lg text-destructive">{adult.bloodGroup || 'Not set'}</p>
                </div>
                <div className="p-3 rounded-lg bg-muted/50">
              ...
              <div className="p-4 rounded-lg border border-destructive/30 bg-destructive/5">
                ...
                <div className="grid grid-cols-3 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{adult.emergencyContact.name || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Relationship</p>
                    <p className="font-medium">{adult.emergencyContact.relationship || 'Not set'}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Phone</p>
                    <p className="font-medium">{adult.emergencyContact.phone || 'Not set'}</p>
                  </div>
                </div>
              </div>
            </CardContent>
        ...
        <TabsContent value="health" className="space-y-4 mt-4">
          <Card className="border-0 shadow-elegant">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Activity className="w-5 h-5" />Medical Conditions</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          {adult.mealPlan && (
            <Card className="border-0 shadow-elegant">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Utensils className="w-5 h-5" />Meal Plan</CardTitle>
                <CardDescription>Created by {adult.mealPlan.createdBy} • {adult.mealPlan.facilityName}</CardDescription>
              </CardHeader>
              <CardContent><p className="text-muted-foreground">{adult.mealPlan.notes}</p></CardContent>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="medications" className="space-y-4 mt-4">
          <Card className="border-0 shadow-elegant">
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><Pill className="w-5 h-5" />Current Medications</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="allergies" className="space-y-4 mt-4">
          <Card className="border-0 shadow-elegant">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-destructive"><AlertTriangle className="w-5 h-5" />Known Allergies</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2 mb-4">
                {adult.allergies.length > 0 ? adult.allergies.map((allergy) => (
                  <Badge key={allergy} variant="destructive" className="text-base py-1 px-3">{allergy}</Badge>
                )) : (
